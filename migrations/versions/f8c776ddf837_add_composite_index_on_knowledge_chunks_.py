...
def upgrade() -> None:
    ...
    # DROP INDEX CONCURRENTLY를 실행해야만 풀림). 로컬 Postgres 16으로
    # 직접 재현했다(179라운드, 8a9d7f4d33d6에도 같은 이유로 적용) - 이
    # CREATE INDEX CONCURRENTLY가 다른 트랜잭션을 기다리는 도중 그 백엔드를
    # 강제 종료하면 indisvalid=false/indisready=false인 인덱스가 남고,
    # 그 상태로 `alembic upgrade head`를 다시 돌리면 정확히 이
    # DuplicateTableError로 막힌다(IF NOT EXISTS를 붙이면 조용히 스킵될
    ...
    with op.get_context().autocommit_block():
        op.execute("DROP INDEX CONCURRENTLY IF EXISTS ix_knowledge_chunks_user_id_embedding_model_created_at")
        op.create_index(
