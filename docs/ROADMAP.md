...
- [x] 216. RAG 검색(`RagService._cosine_similarity`)이 차원이 다른(길이가
      다른) 두 임베딩 벡터를 조용히 비교 가능한 것처럼 채점해, 우연히
      완벽한 점수(1.0)를 받은 차원 불일치 벡터가 진짜 관련 있는 기록을
      밀어내고 검색 결과 1등을 차지할 수 있던 문제 해소 - 89라운드가
      이미 후보로 남겼다가(`docs/ROADMAP.md`의 90라운드 항목이 그 사실을
      기록해둠) 당시 더 급한 이벤트루프 블로킹 문제(90라운드가 고침)에
      밀려 100라운드 넘게 방치됐던 항목이다.

      `KnowledgeChunkRepository.list_for_user()`는 `embedding_model`
      "문자열"이 같은 청크만 후보로 걸러줄 뿐, 그 문자열의 docstring이
      스스로 밝히듯("모델이 다르면 임베딩 공간이 달라 코사인 유사도 비교
      자체가 의미 없다") 실제 벡터 길이가 같다는 보장은 어디에도 없다 -
      `knowledge_chunks.embedding` 컬럼은 의도적으로 pgvector가 아닌
      평범한 `JSON`이라(그 이유가 이미 주석으로 남아 있음) DB 레벨의
      차원 제약도 없다. `EMBEDDING_MODEL` 설정을 기존 색인 재생성 없이
      바꾸거나, 같은 태그의 모델이 다른 출력 차원으로 재배포되면 차원이
      다른 벡터가 같은 `embedding_model` 문자열로 섞여 남을 수 있는데,
      `_cosine_similarity`의 `zip(a, b)`는 길이가 다른 두 리스트를 짧은
      쪽에 맞춰 조용히 잘라서 내적을 구하면서도, `norm_a`/`norm_b`는
      원래(안 잘린) 길이로 계산한다 - 비교가 애초에 불가능한 두 벡터를
      비교 가능한 것처럼 채점해버린다.

      실제 함수(`_cosine_similarity`/`_rank_top_k`)를 그대로 호출해
      재현했다 - 4차원 질문 벡터와, 우연히 앞 두 성분만 질문과 같은
      2차원 "가짜" 청크, 그리고 진짜로 의미상 가까운 4차원 청크를 함께
      채점시키면 2차원 가짜 청크가 `1.0`(완벽한 점수)을 받아 `top_k=1`
      결과에서 진짜 관련 있는 청크를 밀어내고 1등을 차지했다. 아무
      예외도, 로그도 없이 조용히 벌어지는 데이터 왜곡이라 - 이 RAG
      결과는 그대로 학습챗/면접연습 그라운딩 프롬프트에 꽂혀, 사용자
      본인의 관련 없는(심지어 의미 없는) 과거 기록이 진짜 관련 있는
      기록인 것처럼 AI 답변에 섞여 들어갈 수 있었다.

      `_rank_top_k`에서 각 후보를 채점하기 전에 `len(embedding) !=
      len(query_embedding)`이면 그 후보를 조용히 건너뛰고(경고 로그만
      남김) 채점 대상에서 아예 제외하도록 고쳤다 - 짧은 쪽에 맞춰
      자르는 `zip`의 암묵적 동작에 기대는 대신, 비교 불가능한 후보를
      명시적으로 배제한다. 부수적으로, `retrieve_relevant()`의 세
      단계(DB 조회/임베딩 호출/랭킹) 중 189라운드가 이미 DB 조회를,
      기존 코드가 임베딩 호출을 각각 예외로부터 보호하고 있었는데
      정작 마지막 랭킹 단계(`asyncio.to_thread(_rank_top_k, ...)`)만
      보호가 없었던 것도 같은 김에 189라운드와 같은 패턴(`try/except
      Exception: logger.exception(...); return []`)으로 마저 채워
      이 메서드의 모든 단계가 예상 못한 오류를 일관되게 삼키게 했다.

      `tests/test_rag_service.py`에 3개 추가했다 - (1) `_rank_top_k`
      단위 테스트로 차원이 다른 후보가 채점에서 제외되는지, (2)
      `KnowledgeChunkRepository.create()`로 실제 DB에 차원이 다른 청크를
      직접 심어(embedding 길이를 정확히 통제하기 위해 `index_content()`를
      거치지 않음) `retrieve_relevant()`를 end-to-end로 호출했을 때도
      같은 결과가 나오는지, (3) `_rank_top_k`가 예상 못한 예외를 던지도록
      흉내내 그 예외가 새어나가지 않고 빈 리스트로 처리되는지. `git
      stash`로 `rag_service.py` 수정만 되돌리면 세 테스트 다 정확히
      위에서 재현한 것과 똑같은 증상(차원 불일치 청크가 1등을 차지함
      / 랭킹 예외가 그대로 새어나감)으로 실패하는 것까지 확인했다.

      전체 620개 테스트 통과(617 → 620), `services/rag_service.py`
      커버리지 100% 유지, `mypy app tests scripts` 클린. 클라이언트에게
      보내는 응답은 전혀 안 바뀌는(검색 결과의 정확도만 개선되고, RAG
      실패 시 그라운딩이 조용히 빠지는 기존 동작은 그대로인) 순수
