...
- [ ] 19. Refresh token 재사용 탐지 — 지금은 단순 rotation만 있고(사용된
...
- [ ] 20. Ollama 모델 목록 응답 캐싱 — `/api/v1/models`가 요청마다 Ollama
      ...
      캐시로 폴백하는 방식 고려할 것.
- [ ] 21. 구조화된 에러 응답 포맷 — 지금은 FastAPI 기본 `{"detail": "..."}`
      ...
      들지 말 것.
- [ ] 22. 퀴즈 제출 중복/멱등성 처리 — 네트워크 재시도 등으로 같은 제출이
      ...
      정하고 근거를 남길 것.
- [ ] 23. 퀴즈 콘텐츠 RAG 색인 대상 포함 검토 — 학습챗 메시지/면접연습
