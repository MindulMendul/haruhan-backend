...
- [ ] 1. 면접연습(interview practice)에도 RAG 그라운딩 확장 — 학습챗에만 있는 RAG를
      면접연습 질문/피드백에도 적용, 사용자의 과거 학습/복기 내용을 참고하게 함
- [ ] 2. 게스트 → 실계정 전환(계정 연결) API — 게스트로 쌓은 데이터를 유지한 채
      email/password를 등록해 실계정으로 승격
- [ ] 3. 학습챗 세션 / 면접복기 목록 페이지네이션
- [ ] 4. RagService 색인 실패 로깅 추가 (지금은 조용히 무시함)
- [ ] 5. 만료된 refresh token 정리 스케줄러 job 추가 (APScheduler)
- [ ] 6. Redis 레이트리밋 실제 전환 (구조는 있음, 연결만 안 되어 있음)
- [ ] 7. 429 응답에 Retry-After 헤더 추가
- [ ] 8. 퀴즈 생성 JSON 파싱 실패 시 자동 재시도 (1회)
- [ ] 9. CI에 타입체크(mypy) 도입
