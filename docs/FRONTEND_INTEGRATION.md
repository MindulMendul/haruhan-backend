...
- `email`은 서버에서 대소문자를 구분하지 않도록 자동으로 소문자로 정규화합니다(회원가입/로그인/게스트 승격/`PATCH /users/me` 공통). `User@Example.com`으로 가입했어도 `user@example.com`으로 로그인하면 되고, 대소문자만 다른 이메일로는 중복 가입도 `409`로 막힙니다.
...
- `password`를 바꾸면 (1-8의 `DELETE /api/v1/auth/sessions` 전체 로그아웃과 마찬가지로) 이 계정의 모든 refresh_token이 즉시 폐기됩니다 - **지금 요청을 보낸 이 기기도 포함**됩니다. `200` 응답을 받은 직후 프론트에서도 로그인 화면으로 보내야 합니다 (access_token은 만료 전까지 계속 유효하니 그것도 함께 버리세요). `email`만 바꾸는 경우는 세션이 유지됩니다.
...
```
GET /api/v1/auth/sessions?limit=20&offset=0  → [{ id, created_at, expires_at }, ...] (응답 헤더에 X-Total-Count)
DELETE /api/v1/auth/sessions/{id}      → 204 (해당 세션만 로그아웃)
DELETE /api/v1/auth/sessions           → 204 (모든 기기에서 로그아웃)
```
- 여기서 "세션"은 폐기/만료되지 않은 refresh_token 하나를 의미합니다. 비밀번호를 바꾸면 1-6에서처럼 이미 전체 세션이 자동으로 폐기되니, 이 API는 비밀번호는 그대로 둔 채 "다른 기기에서 로그인된 것 같다"며 세션만 강제로 끊고 싶은 경우에 씁니다.
- `GET /api/v1/auth/sessions`는 다른 목록 API와 동일하게 `?limit=20&offset=0`을 받고(`limit` 기본 20, 최대 100) `X-Total-Count`로 전체 개수를 알려줍니다 - 로그인할 때마다 새 세션이 생기고 명시적으로 로그아웃하기 전까지는 안 지워지므로, 반복 로그인하면 계속 쌓일 수 있습니다. 이 API도 다른 `/auth/*` 라우트와 동일하게 `auth_rate_limit`(기본 분당 5회) 대상입니다.
...
조회되는 전체 메시지 히스토리 자체는 그대로 다 보존되지만, AI가 답변을 만들 때
실제로 "기억"하는 범위는 최근 대화까지만입니다. 아주 긴 대화에서 훨씬 이전에
나눴던 내용을 AI가 언급 못 해도 버그가 아니라 의도된 동작입니다.
...
- 연결에 쓴 access token이 `ACCESS_TOKEN_EXPIRE_MINUTES`(기본 30분)로 만료되면,
  연결을 오래 유지한 채로(활발히 메시지를 주고받아 유휴 타임아웃에 걸리지 않고)
  있어도 서버가 다음 메시지 처리 전에 매번 만료 여부를 다시 확인해 연결을
  끊습니다(코드 1008, REST 요청이 이미 만료된 토큰으로 401을 받는 것과 같은
  경계). 오래 켜둔 탭에서 이 코드로 연결이 끊기면, `POST /auth/refresh`로 새
  access token을 받아 재연결하면 됩니다.
- 학습챗/면접복기 스트리밍을 합쳐 서버 전체에서 동시에 열 수 있는 WebSocket
  연결 수에 상한(`MAX_CONCURRENT_WS_CONNECTIONS`, 기본 6)이 있습니다 - 초과
  시도는 accept 전에 코드 `1013`으로 거부됩니다. 정상적인 단일 사용자
  사용 패턴에서는 거의 마주칠 일이 없고(탭 하나당 연결 하나가 보통이라),
  마주친다면 잠시 후 재시도하도록 처리해두면 됩니다.
...
제출도 퀴즈 생성과 같은 `chat_rate_limit`(기본 분당 10회) 대상입니다 - LLM을 호출하진 않지만, 재도전 자체는 정상 기능이라도 한도 없이 반복 제출되면 시도 기록이 무제한으로 쌓일 수 있어 같은 제한을 적용합니다.
...
| PATCH | `/interview/practice-sessions/{id}` | `{ "topic": "새 주제" }` → 주제만 변경 |
...
- `interview_date`는 미래 날짜를 보내면 생성/수정 둘 다 `422`로 거부됩니다(면접 복기는
  이미 치른 면접을 되짚는 기능이라). 오늘 날짜는 허용됩니다 - 서버가 UTC 기준
  "오늘"과 비교하되 KST 등 UTC보다 앞선 시간대와의 어긋남을 흡수하도록 하루의
  여유를 두므로, 서버 UTC 기준으로는 "내일"인 값(=KST 사용자의 실제 오늘)까지도
  정상 허용됩니다. 이 하루 여유를 넘는 값만 거부 대상입니다.
...
```json
{
  ...
  "user_id": "...",
  "user": { "id": "...", "email": "...", "created_at": "...", "is_guest": false },
...
```
본인 소유 기록 전체를 한 번에 JSON으로 내려받습니다. `user`는 `GET /users/me`와 같은 형태(`id`/`email`/`created_at`/`is_guest`)이고, `user_id`는 하위 호환을 위해 남아있는 필드로 항상 `user.id`와 같은 값입니다. 퀴즈 문제에는 (풀이용 목록 조회와 달리) `correct_answer`/`explanation`이 그대로 포함됩니다. 퀴즈의 `source_text`는 직접 붙여넣어 만든 퀴즈에서만 원본 텍스트가 채워지고, `study_session_id`로 만든 퀴즈는 원본이 이미 `study_sessions` 쪽 메시지에 들어있으므로 중복 없이 `null`입니다. 파일 다운로드로 만들고 싶으면 프론트에서 이 응답을 그대로 Blob으로 감싸서 저장하면 됩니다 (서버가 `Content-Disposition`을 붙여주진 않음).
