...
이 API는 쿠키가 아니라 **Bearer 토큰** 방식입니다. 로그인 후 받은 `access_token`을 매 요청의 `Authorization` 헤더에 실어야 합니다.
...
```
GET /api/v1/users/me                       → { id, email, created_at }
...
프론트에 지금 로그인이 아예 없다고 하셨으니, 이 순서로 만드시면 됩니다:

1. 로그인/회원가입 폼 → `/auth/signup`, `/auth/login` → 토큰을 저장(예: 메모리 + `localStorage`)
2. 모든 API 호출에 `Authorization` 헤더 붙이는 공통 fetch 래퍼 작성
3. 그 래퍼에 401 감지 → refresh → 재시도 로직 넣기
